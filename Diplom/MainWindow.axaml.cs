...
public partial class MainWindow : Window
{
    ...
    private void Logout(object? sender, RoutedEventArgs e) => UpdateUIForGuest();

    private async void OpenLocalUpload(object? sender, RoutedEventArgs e)
    {
        if (_currentUser == null) return;
        await new LocalUploadWindow(_currentUser).ShowDialog(this);
    }

    private async void OpenNetworkUpload(object? sender, RoutedEventArgs e)
    {
        if (_currentUser == null) return;
        await new NetworkUploadWindow(_currentUser).ShowDialog(this);
    }

    private async void OpenActiveOrders(object? sender, RoutedEventArgs e)
    {
        if (_currentUser == null) return;
        await new ActiveOrdersWindow(_currentUser).ShowDialog(this);
    }

    private async void OpenHistory(object? sender, RoutedEventArgs e)
    {
        if (_currentUser == null) return;
        await new HistoryWindow(_currentUser).ShowDialog(this);
    }
